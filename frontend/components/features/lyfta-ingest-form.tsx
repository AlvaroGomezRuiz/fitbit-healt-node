...
import { lyftaIngestCardTitle } from "@/lib/ui/single-user-placeholders";
  ...
  return (
    <div className="flex flex-col gap-3">
      <form action={formAction} className="flex flex-col gap-2 rounded-lg border border-border bg-card p-3">
        <h2 className="text-sm font-medium text-foreground">{lyftaIngestCardTitle}</h2>
        ...
        <textarea
          ...
          required
          rows={6}
          className="min-h-32 w-full resize-y rounded-md border border-input bg-background px-2 py-2 text-sm text-foreground outline-none ring-offset-background focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
          placeholder="Pega aquí el contenido exportado o copiado de Lyfta…"
        />
        <button
          type="submit"
          disabled={isPending}
          className="inline-flex h-9 min-h-9 items-center justify-center rounded-md bg-secondary px-3 text-sm font-medium text-secondary-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:opacity-50"
        >
        ...
        <div className="flex flex-col gap-2 rounded-lg border border-border bg-card p-3">
          ...
          <form action={analyzeAction} className="flex flex-col gap-2">
            ...
            <button
              ...
              className="inline-flex h-9 min-h-9 items-center justify-center rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:opacity-50"
            >
    ...
    <div className={`flex flex-col gap-1 text-xs ${tone}`} aria-live="polite">
      ...
      {"structured" in state ? (
        <ul className="list-inside list-disc text-muted-foreground">
          <li>Título (auto): {state.structured.sessionTitle}</li>
          <li>Grupo detectado: {state.structured.grupo}</li>
