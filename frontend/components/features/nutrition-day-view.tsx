...
import {
  emptyDailyNutritionPlan,
  emptyProfilePersonalNutrition,
  nutritionPlanIaBulletDiary,
} from "@/lib/ui/single-user-placeholders";
import { NutritionBannedItemsSection } from "@/components/features/nutricion/nutrition-banned-items-section";
import { NutritionMemoryNotesSection } from "@/components/features/nutricion/nutrition-memory-notes-section";
import { NutritionMenuWeekSection } from "@/components/features/nutricion/nutrition-menu-week-section";
import { NutritionShoppingWeekSection } from "@/components/features/nutricion/nutrition-shopping-week-section";
import { NutritionWeekHistorySection } from "@/components/features/nutricion/nutrition-week-history-section";
...
export function NutritionDayView({ payload }: NutritionDayViewProps): React.ReactElement {
  ...
  return (
    <section className="flex flex-col gap-4" aria-labelledby="nutrition-heading">
      ...
      <NutritionWeekHistorySection block={payload.nutritionWeekHistory} semanaActual={payload.semanaInicioMadrid} />
      <NutritionShoppingWeekSection
        block={payload.nutritionShoppingWeek}
        semanaInicio={payload.semanaInicioMadrid}
        canWrite={payload.usuarioAutenticado}
      />
      <NutritionMenuWeekSection block={payload.nutritionMenuWeek} />
      <NutritionMemoryNotesSection block={payload.nutritionMemoryNotes} canWrite={payload.usuarioAutenticado} />
      <NutritionBannedItemsSection block={payload.nutritionBannedItems} canWrite={payload.usuarioAutenticado} />
      <div className="flex flex-col gap-3 rounded-lg border border-border bg-card p-4">
        ...
        {payload.biometria.state === "ok" ? (
          ...
          <EmptyNote>{emptyProfilePersonalNutrition}</EmptyNote>
        ) : (
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-border bg-card p-4">
        ...
        <ul className="list-disc space-y-1 pl-4 text-xs text-muted-foreground">
          ...
          <li>{nutritionPlanIaBulletDiary}</li>
        </ul>
        {payload.diarioPlanIa.state === "ok" ? (
          ...
          <p className="text-sm text-muted-foreground">{emptyDailyNutritionPlan}</p>
        ) : (
