...
/**
 * Hora civil actual (0–23) en `Europe/Madrid`.
 */
export function currentHourEuropeMadrid(now: Date = new Date()): number {
  ...
  const raw = hourPart?.value;
  const h = raw !== undefined ? Number.parseInt(raw, 10) : Number.NaN;
