...
export function HealthTelemetriaPanel(props: HealthTelemetriaPanelProps): React.ReactElement {
  const { fechas, syncEnabled, result } = props;
        ...
        >
          ...
          <p className="mt-1 text-muted-foreground">
            ...
            {result.code === "query_error" || result.code === "row_shape" ? (
              <>
                Comprueba sesión Supabase Auth o políticas RLS sobre{" "}
                <code className="rounded bg-muted px-1 text-foreground">telemetria_diaria</code>.
              </>
            ) : null}
          </p>
        </div>
        ...
        <>
          ...
          {result.rows.length === 0 ? (
            <p className="text-sm text-muted-foreground" role="status">
              No hay filas en <span className="font-mono text-foreground">telemetria_diaria</span> para este rango.
            </p>
          ) : (
